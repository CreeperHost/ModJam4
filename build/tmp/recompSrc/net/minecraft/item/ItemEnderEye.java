...
public class ItemEnderEye extends Item
{
    ...
    public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
    {
        ...
        if (par2EntityPlayer.canPlayerEdit(par4, par5, par6, par7, par1ItemStack) && block == Blocks.end_portal_frame && !BlockEndPortalFrame.isEnderEyeInserted(i1))
        {
            ...
            else
            {
                ...
                for (l1 = -2; l1 <= 2; ++l1)
                {
                    ...
                    if (par3World.getBlock(l2, par5, i2) == Blocks.end_portal_frame)
                    {
                        if (!BlockEndPortalFrame.isEnderEyeInserted(par3World.getBlockMetadata(l2, par5, i2)))
                        {
                            flag = false;
                            break;
                        }
                ...
                if (flag && k1 == j2 + 2)
                {
                    for (l1 = j2; l1 <= k1; ++l1)
                    {
                        ...
                        if (par3World.getBlock(l2, par5, i2) != Blocks.end_portal_frame || !BlockEndPortalFrame.isEnderEyeInserted(par3World.getBlockMetadata(l2, par5, i2)))
                        {
                            flag = false;
                            break;
                        }
                    ...
                    for (l1 = j2 - 1; l1 <= k1 + 1; l1 += 4)
                    {
                        for (l2 = 1; l2 <= 3; ++l2)
                        {
                            ...
                            if (par3World.getBlock(i2, par5, i3) != Blocks.end_portal_frame || !BlockEndPortalFrame.isEnderEyeInserted(par3World.getBlockMetadata(i2, par5, i3)))
                            {
                                flag = false;
                                break;
                            }
                    ...
                    if (flag)
                    {
                        for (l1 = j2; l1 <= k1; ++l1)
                        {
                            for (l2 = 1; l2 <= 3; ++l2)
                            {
                                ...
                                i2 += Direction.offsetX[j1] * l2;
                                i3 += Direction.offsetZ[j1] * l2;
                                par3World.setBlock(i2, par5, i3, Blocks.end_portal, 0, 2);
                            }
                        }
                    }
                }
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer)
    {
        ...
        if (movingobjectposition != null && movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && par2World.getBlock(movingobjectposition.blockX, movingobjectposition.blockY, movingobjectposition.blockZ) == Blocks.end_portal_frame)
        {
